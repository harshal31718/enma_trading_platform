...
| 10 | Monte Carlo Optimiser & Strategy Lab | PBO overfitting stat (needs per-trial OOS eval, real separate scope), Phase 4 (MC-scored selection) — Phase 1 (job plumbing) + Phase 2 (MC tab) + Phase 3a (walk-forward job plumbing) + Phase 3c (Optimizer tab UI, fold-level) + Phase 3d (per-trial persistence + trials table UI) + Phase 3b (Optuna/TPE Bayesian search, 1 pre-existing bug fixed — inf-loss JSON crash) + Phase 3e (Deflated Sharpe Ratio, fully live-verified incl. 1 real unrelated bug found/fixed — `_build_param_grid` OOM/hang on large grids, e.g. AdaptiveTrend's default Optimizer-wizard grid) shipped 2026-07-19 | In progress | P1 | 9 (9.1/9.3, shipped) | 2026-07-19 |
