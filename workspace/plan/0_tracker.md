...
| 8  | Governance, correctness & cleanup | **8.2 SHIPPED 2026-07-21** (SEC-10 schema-validation layer + symbol-key whitelisting on the real `positionDetails.${symbol}` injection site). **8.3 SHIPPED 2026-07-21** (ENG-8 warmup math — 2 of 5 seeded strategies were already permanently stuck in "warming up" with default config; fixed by reading each strategy's own already-correct `MIN_WARMUP_CANDLES` instead of a PARAMS-largest-numeric-value scan). **8.4 SHIPPED 2026-07-21** (ENG-15 candle column-order safety — new `core/candle_columns.py` canonical constants + shared `build_candle_array()`, migrated ~50 production call sites off bare positional literals across kernel.py/market_data_feed.py/strategy.py/live_bot_manager.py/backtest_runner.py/models/cost.py/2 strategy files; closed a real test gap — `fetch_htf_candles` had zero OHLC-swap coverage — see plan file). Remaining: 8.5, 8.7 (SYS-3 doc item carried from F6) — 8.6 verified-already-shipped 2026-07-18, no code needed | In progress | P3 | 3 (done), 5, 6 | 2026-07-21 |
