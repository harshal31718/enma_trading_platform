...
| 6  | Engine decomposition & exchange abstraction | 6.1 (OrderRouter still open — needs execute_entry/reduce/exit/flip business-logic split, materially harder than the 4 shipped extractions); 6.2 (Exchange interface half shipped — migrating ~24 existing call sites to actually use it is still open); 6.4 (LiveAdapter half shipped — no more `self.manager.<attr>` chains; kernel `is_live` removal still open, separate higher-risk pass); 6.5 (pooled-client half shipped; Redis-stream ordered/at-least-once channel — needs Node-side consumer too — still open); 6.3/6.6 not started | In progress | P2 | 5 (shipped 2026-07-18), 21.3/21.4 (shipped 2026-07-17) — unblocked | 2026-07-20 |
