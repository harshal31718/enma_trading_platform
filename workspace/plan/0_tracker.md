...
|----|-------|-----------------|--------|----------|------------|---------|
| 21 | Live algo industry-standard audit — **fixes** | **ALL SHIPPED IN CODE 2026-07-20** — 21.5c (batched reconcile) shipped, container-verified [653/653 pytest, golden-master byte-identical] (21.1–21.4 + 21.5a/b + all of 21.7 [A-11/A-12/A-13/A-14] shipped, container-verified 2026-07-17 [301/301 pytest]; 21.6 → Merged→22.1). **Live Testnet re-verification 2026-07-24 ran a real session and found a real gap**: `-2021 Order would immediately trigger` was firing repeatedly (not the rare edge case F7 assumed), triggering M-5's emergency-close in a real entry→reject→close→re-entry loop (-$54.96 testnet). Fixed with a proactive `enforce_min_trigger_distance()` floor (live-only, zero golden-master risk) — see DECISIONS.md #32, `0_fixes-queue.md` F7. Engine pytest 691/691. Bracket-cancel (A-4/A-5), naked-position re-arm (A-7), and fill-latency (A-8) themselves were not independently re-exercised this pass (the session never got far enough into a stable position to test those specifically) — still nominally pending a cleaner live run. | **Done** (F7's -2021 gap closed; A-4/A-5/A-7/A-8 still pending a clean live re-run) | — | — | 2026-07-24 |
| 5  | Live-trading state integrity | ALL SHIPPED (5.1–5.6, container-verified 2026-07-18 [engine 441/441, server 112/112]) — 5.5 scoped to running-total accumulation sites (documented scope decision, not full float→Decimal); 5.6's resume capability + `RESUME_SESSIONS_ON_RESTART` toggle shipped, default OFF **reaffirmed 2026-07-24** (DECISIONS.md #30) — flatten-on-restart stays the platform default by deliberate choice, not an oversight | **Done** | — | 21.1–21.2 informed 5.6 | 2026-07-24 |
...
| 9  | Backtest & optimizer correctness (quant core) | ALL STEPS SHIPPED (9.1–9.11, container-verified 2026-07-18 [427/427 pytest]) — liquidation fee (QNT-4) shipped opt-in 2026-07-18 per user decision; warmup fail-loud (QNT-16) shipped 2026-07-24 [675/675 pytest, golden-master byte-identical], unblocked by Plan 8; `"inf"`-string persistence stays deliberately deferred (dormant, no active consumer) | **Done** | — | — | 2026-07-24 |
...
| 8  | Governance, correctness & cleanup | **ALL STEPS SHIPPED 2026-07-21** — 8.1 (2026-07-16) + 8.2 (SEC-10 schema-validation + symbol-key whitelisting on the real `positionDetails.${symbol}` injection site) + 8.3 (ENG-8 warmup math — 2 of 5 seeded strategies were permanently stuck in "warming up" with default config, fixed) + 8.4 (ENG-15 candle column-order safety — new `core/candle_columns.py`, ~50 call sites migrated) + 8.5 (ENG-18 exit-order policy verified already-documented/configurable from Plan 9; ENG-14 strategy_name validation + load/param failure visibility, the real gap, fixed) + 8.6 (verified-already-shipped 2026-07-18) + 8.7 (SEC-8 `/health` Redis connection reuse) + the SYS-3 named-volume doc item 8.1 deferred, closed same day. See plan file for full detail. | **Done** | — | 3 (done), 5, 6 | 2026-07-21 |
| 23 | New strategy: high-risk/high-leverage breakout scalper ("MarginSurge") | **VALIDATION RUN COMPLETE 2026-07-21 — verdict: DO NOT SHIP.** Implemented per spec, ran through gates 1-4 (cost-realism, regime/diagnostic backtest, lookahead sentinel, grid-optimize+OOS) — negative expectancy on every tested symbol/timeframe, grid-search OOS split inverted sign (textbook overfit). Gates 5-6 (Monte Carlo/leverage selection, chaos stress) deliberately not run — gate 4's OOS failure is itself a kill per the plan's own gate ordering. Code kept as a boundary-clean/lookahead-clean reference implementation, seeded with its validation outcome stated explicitly; not recommended for live or further deployment as specified. See `DECISIONS.md` #29 and `workspace/docs/strategies/MarginSurge.md` for the full report. | **Done** (rejected by its own gates — a valid terminal outcome, not abandoned mid-work) | — | 21 (live phase, moot — see verdict), 22.1–22.2 (moot) | 2026-07-21 |
| 24 | BestSupertrend fixes (never trades at defaults) | ALL SHIPPED (S-1 through S-5, container-verified 2026-07-17 [353/353 pytest], golden-master re-baselined for S-1 — only BestSupertrend diverges, other 4 strategies byte-identical). **Live Testnet re-verification 2026-07-24 found and fixed a real regression**: `server/src/controllers/algo.controller.js`'s Chaos-mode `CHAOS_LAUNCH_LIST` still used the pre-S-4 param key `order_type` instead of the renamed `direction_filter` — every Chaos-launched BestSupertrend symbol was hitting S-4's own "reject unknown params" guard (F-016) and permanently stuck in WATCHING, never trading. Fixed (server jest 269/269); the other 4 Chaos-launch strategies' param keys were cross-checked against their live PARAMS schemas and are all still correct. | **Done** | — | — | 2026-07-24 |
