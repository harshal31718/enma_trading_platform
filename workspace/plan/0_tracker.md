...
| 10 | Monte Carlo Optimiser & Strategy Lab | DSR/PBO overfitting stats, Phase 4 (MC-scored selection) — Phase 1 (job plumbing) + Phase 2 (MC tab) + Phase 3a (walk-forward job plumbing) + Phase 3c (Optimizer tab UI, fold-level) + Phase 3d (per-trial persistence + trials table UI) + Phase 3b (Optuna/TPE Bayesian search, fully live-verified incl. 1 real pre-existing bug found/fixed — inf-loss JSON crash on `/optimize/run`, affected grid too) shipped 2026-07-19 | In progress | P1 | 9 (9.1/9.3, shipped) | 2026-07-19 |
