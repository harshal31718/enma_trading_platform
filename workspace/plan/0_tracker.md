...
| 10 | Monte Carlo Optimiser & Strategy Lab | PBO overfitting stat — **scoped 2026-07-19, needs its own design pass, not started** — everything else in Plan 10 including all of Phase 4b now shipped: Phase 1 (job plumbing) + Phase 2 (MC tab) + Phase 3a (walk-forward job plumbing) + Phase 3c (Optimizer tab UI, fold-level) + Phase 3d (per-trial persistence + trials table UI) + Phase 3b (Optuna/TPE Bayesian search, 1 pre-existing bug fixed — inf-loss JSON crash) + Phase 3e (Deflated Sharpe Ratio, fully live-verified incl. 1 real unrelated bug found/fixed — `_build_param_grid` OOM/hang on large grids) + Phase 4a (MC-scored trial selection, dead-code API gap in `labConfig.js` found/fixed) + Phase 4b's copy-to-backtest slice (`RobustPickPanel.jsx`) + Phase 4b's backtest-page MC auto-enqueue strip (`MCSummaryStrip.jsx`, §4.4) + Phase 4b's risk_pct/leverage search (`_build_combined_grid`, engine+server+client) shipped 2026-07-19, **real-Docker-verified same day** (a second concurrent session): engine 551/551, server jest 169/169, client `vite build` clean + vitest 11/11, live-verified via a real `/lab` Optimizer wizard browser session | In progress | P1 | 9 (9.1/9.3, shipped) | 2026-07-19 |
