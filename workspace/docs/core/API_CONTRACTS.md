...
- **`GET /api/v1/settings/exchange`** -> `{ takerFee, makerFee, slippagePct, fundingEnabled, fundingRate, defaultCapital, defaultLeverage, defaultBotCapital, defaultBotLeverage, minEdgeMult, riskPct, riskRewardRatio, maxSessionDrawdown, liqBufferPct, chaosMaxStrategies, chaosMaxManualSymbols, chaosDefaultCapital, chaosDefaultLeverage, chaosDefaultTimeframe }`
- **`PUT /api/v1/settings/exchange`** -> Req: same field set as GET, all optional -> same shape as GET
