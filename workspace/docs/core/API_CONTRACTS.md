...
- **`GET /api/v1/auth/google/callback`** -> Google redirects here. **Login is open** — any valid Google account succeeds: signs a 7-day JWT (`{ userId, role }`), sets it as `enma_jwt` (`httpOnly`, `sameSite: lax`, `secure` in prod), redirects to `CLIENT_URL/`. On a genuine OAuth failure: redirects to `CLIENT_URL/login?error=auth_failed`.
- **`POST /api/v1/auth/logout`** -> clears the `enma_jwt` cookie -> `{ success: true, data: null }`
- **`GET /api/v1/auth/me`** -> requires `verifyJWT` -> `{ id, email, name, avatar, role: "user"|"admin", algoAccess: "none"|"requested"|"granted" }`

### Admin (admin-only, `requireAdmin` middleware)
- **`GET /api/v1/admin/users`** -> `{ success: true, data: { id, email, name, avatar, role, algoAccess: "none"|"requested"|"granted", requestedAt, createdAt, lastLoginAt }[] }` — all users (no `userId` scope; deliberate admin exception).
- **`PATCH /api/v1/admin/users/:id/algo-access`** -> Req: `{ status: "granted"|"none" }` -> `{ id, algoAccess }`. 400 `VALIDATION_ERROR` if status invalid; 404 `NOT_FOUND` if user missing; 403 `FORBIDDEN` if the target is an admin.
...
```
- **`POST /api/v1/algo/access-request`** -> `{ algoAccess: "requested"|"granted" }` — user requests Algo Trading access (idempotent; only flips `none`→`requested`). Not gated.
- **`POST /api/v1/algo/sessions`** [**`requireAlgoAccess`** — 403 `ALGO_ACCESS_REQUIRED` if the user is not admin and not granted] -> Req: `{ strategyId, symbols, timeframe, params, capital, leverage }` -> `{ sessionId, status }`. 400 `SYMBOL_CAP_EXCEEDED` if `symbols.length` exceeds `Settings.limits.testnet.maxSymbolsPerBot`; 409 `BOT_LIMIT_REACHED` if the user is already at `Settings.limits.testnet.maxConcurrentBots` running/starting/stopping sessions (2026-07-03, see DECISIONS.md).
