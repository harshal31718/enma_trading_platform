...
// `results` object). `type: 'monte_carlo'` (Plan 10 Phase 1) and
// `'optimization'` (Plan 10 Phase 3a, walk-forward) are both live.
const LabResultSchema = new mongoose.Schema(
  {
    userId: { type: String, required: true, index: true },
    labId: { type: String, required: true, unique: true, index: true },
    type: { type: String, enum: ['monte_carlo', 'optimization'], required: true },
