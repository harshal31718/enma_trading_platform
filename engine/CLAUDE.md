...
│   ├── engine_alias.py       ← `install_engine_alias()` — a `sys.meta_path` finder/loader making `engine.*` imports (the documented strategy-author convention) resolve to the exact same module objects as the canonical `core.*`/`services.*`/etc imports every internal module uses (Plan 6 Step 6.6, ENG-12, 2026-07-20). Replaces the old `/engine -> /app` symlink + `sys.path.insert(0, '/')` hack, which was duplicated 5 times (`main.py` + 4 standalone scripts) and — verified — created a SEPARATE, duplicate module identity for the same source files (`engine.core.margin is core.margin` was `False`) instead of one canonical identity. Call `install_engine_alias()` once, early, in any process that might dynamically load a strategy file — already wired into `main.py`, `scripts/golden_master.py`/`recursive.py`/`lookahead_sentinel.py`/`_portfolio_check.py`, and `tests/conftest.py`; idempotent, safe to call from more than one
...
│   ├── candle_columns.py     ← Plan 8 Step 8.4 (ENG-15): canonical `TIMESTAMP, OPEN, CLOSE, HIGH, LOW, VOLUME = 0..5` column-index constants + `NUM_COLUMNS`/`build_candle_array()` — the single source of truth for the engine's nonstandard `[ts, open, close, high, low, vol]` candle layout. Every consumer (kernel.py, market_data_feed.py, strategy.py, live_bot_manager.py, backtest_runner.py, models/cost.py, strategy files) imports these instead of hardcoding column indices; `engine/indicators/base.py` re-exports the same names for its adapters rather than re-declaring them
...
│   ├── order_router.py       ← OrderRouter — the fifth and final Step 6.1 extraction (ENG-1), 2026-07-20. Six free functions moved verbatim from `live_bot_manager.py` (`fmt_num`/`make_client_id`/`binance_error_detail`/`extract_fill_price`/`query_real_fill_price`/`uuid4_hex8` — `reconciler.py` still imports them unchanged via `from core.live_bot_manager import ...`, re-exported from here) plus a stateless `OrderRouter` class (`place_market_order`/`place_algo_order`/`confirm_fill`) wrapping `Exchange.place_order`/`place_algo_order`/`query_order` (Plan 6 Step 6.2, migrated 2026-07-20 — was a raw `send_signed_request` call) for MARKET/CONDITIONAL(SL/TP) order placement and the "trust avgPrice, else re-query by clientOrderId" fill-confirmation ladder (Plan 5 Step 5.2/5.3, ENG-2/ENG-10). Every method (plus the free `query_real_fill_price`) takes an optional `exchange: Exchange | None = None`, defaulting to `BinanceFuturesTestnet()` — real call sites pass the session's own resolved instance. Deliberately narrow: the risk/governor decision chain and all local session/position state mutation inside `LiveAdapter.execute_entry`/`execute_reduce`/`execute_exit`/`execute_flip` stay in `LiveAdapter` — only the actual order-placement plumbing (repeated across all four methods) moved. `LiveAdapter.__init__` holds `self._order_router = OrderRouter()`
...
│   └── simulate.py        ← POST /simulate/monte-carlo (Plan 10 Phase 1), POST /simulate/optimize (Phase 3a, walk-forward), POST /simulate/pbo (PBO/CSCV) — job-based Strategy Lab endpoints, all write labResults
├── config/
...
│   ├── pbo.py              ← Probability of Backtest Overfitting (CSCV, Bailey/Borwein/López de Prado/Zhu 2015) — closes out Plan 10, deliberately NOT a walk-forward extension (see the file's own header docstring for why). `run_lab_pbo()`: calls `optimizer.run_optimization`/`run_bayesian_optimization` DIRECTLY over the full date range (no fold splitting) to get N candidates, each already backtested once with trades persisted; `compute_pbo()` (pure, no I/O) partitions the range into `nBlocks` (even, default 8, capped 12) contiguous blocks and scores every `C(nBlocks, nBlocks/2)` train/test split by slicing each candidate's already-fetched trades in memory — zero extra backtests beyond the initial N. Scoring statistic is the same trade-level Sharpe-like proxy as DSR/the stitched-OOS aggregate. OOS ranking is ASCENDING (rank 1 = worst, matching the paper's own convention exactly — getting this backwards silently inverts what `P(logit<=0)` means). Writes labResults (`type: "pbo"`), called from routers/simulate.py's `POST /simulate/pbo`.
...
├── tests/
│   ├── conftest.py        ← installs the `engine.*` import alias (`core/engine_alias.py`, Plan 6 Step 6.6) for the pytest process, so tests that dynamically load a strategy file resolve `engine.*` imports correctly
...
│   ├── test_pbo.py         ← `_block_boundaries`/`_assign_block`/`_stat_from_returns` (pure), `compute_pbo`'s CSCV combinatorics verified against two hand-derived EXACT scenarios (perfectly anti-correlated IS/OOS -> pbo==1.0; strictly/uniformly ordered candidates -> pbo==0.0 — not just directional checks), insufficient-data guard, missing-OOS-data-ranks-worst-not-excluded, and an end-to-end `run_lab_pbo` wiring test (Plan 10 PBO)
...
│   ├── test_market_data_feed.py ← `MarketDataFeed` unit tests (Plan 6 Step 6.1) — warmup DB ordering/empty/error paths, REST fetch excludes-open-candle + error path, `append_candle` new/same/older-timestamp + 500-candle trim
│   ├── test_node_notifier.py ← `NodeNotifier` unit tests (Plan 6 Step 6.1) — PATCH headers/URL/payload shape, exception-swallowing (notify is best-effort), `call_internal`'s parsed-JSON success path + error-dict-on-exception path
│   ├── test_session_registry.py ← `SessionRegistry` unit tests (Plan 6 Step 6.1) — per-(session,symbol) lock identity/distinctness, empty-registry defaults, and `LiveBotManager`'s delegating-property facade actually shares the SAME dict objects (not copies) with the registry
│   ├── test_reconciler.py    ← `Reconciler` extraction-seam tests (Plan 6 Step 6.1) — standalone constructibility, correct registry/notifier/manager wiring, and each of `LiveBotManager`'s 7 delegating wrapper methods actually calls through to `self._reconciler`. Deliberately not a re-test of reconcile/cancel/amend/governor-breach behavior — that's covered by the 20+ pre-existing tests (`test_cancel_symbol_algo_orders.py`, `test_maybe_amend_exchange_sl.py`, `test_reconcile_naked_position_rearm.py`, `test_reconcile_fixes.py`, etc.) which still exercise the same code through the old method names
│   ├── test_live_adapter_dependency_wiring.py ← `LiveAdapter` collaborator-resolution tests (Plan 6 Step 6.4, partial) — `self._registry`/`self._notifier`/`self._reconciler` resolve to the SAME instances as `manager`'s own, and patching `Reconciler` (not `LiveBotManager`'s old wrapper methods) is what now actually intercepts adapter calls
│   ├── test_exchange.py      ← `Exchange`/`BinanceFuturesTestnet`/`BinanceFuturesMainnet` contract tests (Plan 6 Step 6.2), parametrized across both implementations — every method's endpoint/HTTP-method/mode routing, listen-key lifecycle, and an explicit "mode can never be overridden by a call site" test. Monkeypatches `services.binance_testnet.send_signed_request` (not `core.exchange`'s own name) — `Exchange._signed` looks it up module-qualified at call time specifically so this works
│   ├── test_order_router.py  ← `OrderRouter` extraction-seam tests (Plan 6 Step 6.1, ENG-1) — the six moved free functions (incl. the F7-regression zero-avgPrice-string case), `place_market_order`/`place_algo_order`'s exact param shape (incl. `reduceOnly` present/absent), `confirm_fill`'s trust-real-avgPrice/re-query-on-zero/still-unconfirmed-returns-None paths, and `LiveAdapter._order_router` wiring. Deliberately not a re-test of execute_entry/reduce/exit/flip behavior — the 20+ pre-existing tests (`test_entry_unconfirmed_fill.py`, `test_execute_entry_bracket_safety.py`, `test_execute_flip_idempotency.py`, etc.) already cover that, now reached via `self._order_router` instead of inline code
│   └── test_exchange_migration.py ← call-site migration wiring tests (Plan 6 Step 6.2, 2026-07-20) — `Reconciler._resolve_exchange`'s default/session-override behavior, `OrderRouter.place_market_order` actually threading an injected mainnet `Exchange`'s mode through (not just the testnet default), `UserDataStreamManager` defaulting to testnet and honoring an injected `Exchange` for both listen-key calls and the WS URL, and `start_session` putting a `BinanceFuturesTestnet` instance on every session. Deliberately not a re-test of reconcile/order-placement/user-data-stream BEHAVIOR — the ~20 pre-existing test files already cover that end-to-end, unchanged, through the same code now reached via `Exchange`
