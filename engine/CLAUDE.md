...
│   └── test_exchange.py      ← `Exchange`/`BinanceFuturesTestnet`/`BinanceFuturesMainnet` contract tests (Plan 6 Step 6.2), parametrized across both implementations — every method's endpoint/HTTP-method/mode routing, listen-key lifecycle, and an explicit "mode can never be overridden by a call site" test
